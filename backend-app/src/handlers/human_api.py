...
def handle_list_questions(event: dict) -> dict:
    """
    ...
    logger.info(f"Listed {len(questions)} open questions")
...
def handle_get_question(event: dict) -> dict:
    ...
    logger.info(f"Retrieved question {question_id} for human")
    
    return success(question.to_human_detail(can_answer=can_answer))


def handle_submit_response(event: dict) -> dict:
    """
    ...
    logger.info(
        f"Submitted response {response.response_id} to question {question_id}. "
        f"New count: {new_count}, status: {new_status}"
    )
